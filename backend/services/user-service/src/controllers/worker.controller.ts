 ...
 */
const calculateAvailableBalance = async (workerId: any): Promise<number> => {
  const bookings = await Booking.find({ worker: workerId, status: 'COMPLETED' }).select('pricing');
  const netEarnings = bookings.reduce(
    (sum, booking) => sum + (booking.pricing.finalPrice || 0) - (booking.pricing.platformFee || 0),
    0
  );
...
export const requestWithdrawal = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!worker.bankDetails?.accountNumber) {
    return errorResponse(
      res,
      'Please add your bank details to your profile before requesting a withdrawal',
  ...
  if (amount < MIN_WITHDRAWAL_AMOUNT) {
    return errorResponse(
      res,
      `Minimum withdrawal amount is Rs. ${MIN_WITHDRAWAL_AMOUNT}`,
      HTTP_STATUS.BAD_REQUEST
    );
  }
  ...
  if (amount > availableBalance) {
    return errorResponse(
      res,
      `Withdrawal amount exceeds available balance (Rs. ${availableBalance})`,
      HTTP_STATUS.BAD_REQUEST
    );
  }
